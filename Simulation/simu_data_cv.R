...
linPred.tc <- beta_0+b_i+beta_2*baseEffect # time constant part of linear predictor

integral.f = function(t, alpha, bt, beta_1, beta_3, baseEffect, gamma_2){ 
  # integral within Survival function
  exp((alpha-1)*log(t) + gamma_2*((beta_1+bt)*exp(log(t)) + beta_3*exp(log(t))*baseEffect))
}

surv.func = function(t, u, linPred.tc, beta_1, beta_3,
                     bt, trt, baseEffect){
  ...
  associ = 0.25 # association coefficient
  baseScale = 0.1 # baseline hazard scale
  baseShape = 0.15 # baseline hazard shape
  gamma_1 = -0.2 # treatment effects on survival
...
######################################
### dataset for time-dependent cox ###
######################################

SurvTdc <- tmerge(SurvDat, SurvDat, id=id, status = event(endpt, event)) #set range
SurvTdc <- tmerge(SurvTdc, LongDat, id=id, biomarker = tdc(time, Y))
