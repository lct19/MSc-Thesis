...
surv.func = function(t, u, dev.tc, beta_2, beta_5,
  ...
  baseShape = 1.8 # baseline hazard shape
  ...
  for (i in 1:N){
    ...
    output[i] = 
      tryCatch(uniroot(function(t) surv.func(t, u = u[i], 
                       ...
                       interval=c(1e-20, 100))$root, # avoid 0
               error=function(e) return(200)) # if t is out of 12, then set 13
  }
  return(output)
}
...
endpt <- pmin(eventTimes, 100) # event time > 12 -- censoring
